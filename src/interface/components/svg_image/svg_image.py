import cairosvg
from PIL import Image, ImageTk
from io import BytesIO
import tempfile
import contextlib
import re
import customtkinter as ctk


class SvgImage(ImageTk.PhotoImage):
    def __init__(
        self,
        svg_path: str,
        stroke: str = "#000000",
        fill: str = "none",
        size: tuple[int, int] = (32, 32),
    ):
        self.size = size
        self.pil_image = self._get_bytes_image(svg_path, stroke, fill)

    def _get_bytes_image(self, svg_path: str, stroke: str, fill: str):
        try:
            with open(svg_path, "r") as file:
                svg_content = file.read()
        except FileNotFoundError:
            raise FileNotFoundError(f"SVG file not found: {svg_path}")

        modified_svg_content = re.sub(
            r'stroke="[^"]+"', f'stroke="{stroke}"', svg_content
        )
        modified_svg_content = re.sub(
            r'fill="[^"]+"', f'fill="{fill}"', modified_svg_content
        )

        # Using a context manager for the temporary file ensures it's cleaned up
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, suffix=".svg"
        ) as temp_file:
            temp_file.write(modified_svg_content)
            temp_svg_path = temp_file.name

        try:
            png_data = cairosvg.svg2png(
                url=temp_svg_path, output_width=self.size[0], output_height=self.size[1]
            )
            if not isinstance(png_data, bytes):
                raise RuntimeError("Failed to convert SVG to PNG")
            return Image.open(BytesIO(png_data))
        finally:
            # Ensure the temporary file is removed
            with contextlib.suppress(OSError):
                import os

                os.remove(temp_svg_path)

    def get_ctk_image(self):
        ctk_image = ctk.CTkImage(light_image=self.pil_image, size=self.size)
        return ctk_image

    def get_tk_image(self):
        tk_image = ImageTk.PhotoImage(image=self.pil_image)
        return tk_image
